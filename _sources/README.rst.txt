...
При обработке файла проверяется:
    ...
    1. Есть ли уже такая задача. Если нет, то она создается, но если в csv файле указан для нее процент выполения 100%, то проверяет выполнена задача в ДПР или нет. Если не выполнена, то устанавливает ей статус "Выполнено", дату тестирования и выполнения - текущую.
